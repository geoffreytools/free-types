...
type From<T extends Mappable<any>, Args extends Keys[] = never, Keys = keyof T> =
    $From<T,
        [Args] extends [never]
        ? T extends unknown[] ? GetIndices<T> : Union2Tuple<Keys>
        : NormalizeArgs<Args>
    >

interface $From<T, Keys extends unknown[]> extends
    Type<Slice<_Object2Tuple<T, Keys>, 0, Keys['length']>>
        { type: FromN<this['arguments'], Keys, T> }

type FromN<This extends unknown[], Keys extends unknown[], T> = {
